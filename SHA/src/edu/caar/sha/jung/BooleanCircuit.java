		...
		while (!edges.isEmpty()) {
			...
			case AND:
				...
				if (values.get(source)) {
					...
					for (Edge edge : getOutEdges(dest)) {
						addEdge(new Edge(), otherSource, getDest(edge),
								EdgeType.DIRECTED);
					}
					removeVertex(dest);
					...
					for (Edge edge : getOutEdges(dest)) {
						newEdge = new Edge();
						addEdge(newEdge, source, getDest(edge),
								EdgeType.DIRECTED);
						edges.add(newEdge);
					}
					removeVertex(dest);
			...
			case XOR:
				...
				if (values.get(source)) {
					...
					not = not(otherSource);
					for (Edge edge : getOutEdges(dest)) {
						addEdge(new Edge(), not, getDest(edge),
								EdgeType.DIRECTED);
					}
				} else {
					// Value at dest depends on other source
					for (Edge edge : getOutEdges(dest)) {
						if (!addEdge(new Edge(), otherSource, getDest(edge),
								EdgeType.DIRECTED)) {
							newEdge = new Edge();
							addEdge(newEdge, source, getDest(edge),
									EdgeType.DIRECTED);
